...
function fireWorks(parent, x, y, r, n) {
    fireWorks_image = fireWorks_image || lg.loadImage('AppRes/res/star.png')
    if (playSound) {
        playSound.stop()
    }
    correct_sound.play()
...
function Active2(sec) {
    ...
    let m = new menuWheel(0, 0, 100, items)
    m.dump = 0.95
    m.distanceofmove = 0
    m.bg = style.bg
...
function Active3(sec) {
    ...
    // //
    bcard = lg.loadImage('AppRes/res/bcard.png') 
    for (let i in rs) {
        let o = rs[i]
        let x = -ox + (i*iw)
...
function Active5(sec) {
    ...
    // pic.filter = GRAY
    pic.drawafter = function(){
        lg.setBlendMode("destination-atop")//destination-in destination-atop
        lg.setColor(rgba(0, 0, 0, 50))
        lg.rectangle('fill', -this.width/2, -this.height/2, this.width, this.height)
    }
    e.insert(pic)
    for (let i in rs) {
        let o = rs[i]
...
function Active12(sec) {
    ...
    e.drawafter = function(){
        ...
        if (this.one) {
            lg.setColor(255)
            lg.line(this.mx, this.my, this.one.x, this.one.y)
        }
...
function Active13(sec) {
    ...
    let Numbers = []
    for (let i = 1; i <= 10; i++) {
        let im = lg.loadImage('AppRes/res/2/' + i + '.png')
        Numbers.push(im)
    }
...
function Active14(sec) {
    ...
    for (var j = 0; j < 3; j++) {
        for (var i = 0; i < 3; i++) {
            ...
            // im.filter(GRAY)
            // pic.filter = GRAY
            pic.image = rs[index].image
            pic.baseImage = rs[index].image
            pic.rect = true
            pic.drawafter = function(){
                if (this.rect) {
                    lg.setBlendMode("destination-atop")//destination-in destination-atop
                    lg.setColor(rgba(0, 0, 0, 60))
                    lg.rectangle('fill', 0, 0, this.width, this.height)
                }
            }
            childs.push(pic)
...
function Active19(sec) {
    ...
    e.target = vec2(0, 0)
    let fishIm = lg.loadImage('AppRes/res/fish.png')
    let fish = new Pictur(fishIm, 0, 0, iw, iw)
    fish.origin = CENTER_CENTER
    fish.hasBg = false
    fish.fixed = true
    fish.imageColor = rgba('hsl(200, 100%, 50%)')
