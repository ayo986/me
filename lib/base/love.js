...
var init = function () {
    ...
    document.addEventListener("touchmove",love.mouse._move, false);
    document.addEventListener("touchstart",love.mouse._downHandler, false);
    document.addEventListener("touchend",love.mouse._upHandler, false);
...
love.graphics.font = {};
love.graphics.scissor = []
love.graphics.modeText = 'fill'
...
love.graphics.loadImage = function(url, f) {
    return this.newImage(url, f) 
}

love.graphics.shadow = function(c, n, offx, offy) {
    this.ctx.shadowColor = c || '#ffffff'
    this.ctx.shadowBlur = n || 0
    this.ctx.shadowOffsetX = offx || 0
    this.ctx.shadowOffsetY = offy || 0
}
//Drawing functions
...
love.graphics.polygon = function () {
    ...
    this.ctx.closePath();
    love.graphics._mode(arguments[0]);
}

// ich habe das gemacht!
love.graphics.polygon2 = function (mode, verts, close) {
    verts = verts || []
    this.ctx.beginPath();
    this.ctx.moveTo(verts[0],verts[1]);
    for (var i = 0; i < verts.length-2; i+=2) {
...
love.graphics.print = function (t,x,y,r,sx,sy,ox,oy) {
    ...
    this.ctx.rotate(r);
    if (this.modeText == 'fill') {
        this.ctx.fillText(t, -ox, -oy+this.font.size);
    }
    // 

    if (this.modeText == 'line') {
        this.ctx.strokeText(t, -ox, -oy+this.font.size);
    }
    this.ctx.restore();
}
...
love.graphics.newImage = function (url, f) {
    // preload image
    ...
    img.getData = function () {
        ...
        let t = 0
        let ctx = love.graphics.ctx 
        drawImage(this, w/2, h/2, w, h)
...
love.graphics.newFont = function (fnt,size) {
    ...
    font.getWidth = function(txt){
        var metrics = love.graphics.ctx.measureText(txt);
        return metrics.width
    }

    font.getHeight = function(txt){
        var metrics = love.graphics.ctx.measureText(txt);
        return parseInt(love.graphics.ctx.font)
    }
    return font;
}
...
love.graphics.saveClip = function () {
    this.ctx.save();
}

love.graphics.shadow = function(c, r, xoff, yoff){
    this.ctx.shadowColor    = c || "#ffffff"
    this.ctx.shadowBlur     = r || 0
    this.ctx.shadowOffsetX  = xoff || 0
    this.ctx.shadowOffsetY  = yoff || 0
}

love.graphics.setClip = function () {
    this.ctx.clip();
}

love.graphics.noClip = function () {
    this.ctx.restore();
}
//Get functions
...
love.graphics.newImageData = function (w, h) {
    return this.ctx.createImageData(w, h)
}

love.graphics.toImage = function (data) {
    // let url = this.canvas.toDataURL()
    let cvs = this.newCanvas(data.width, data.height)
    let ctx = cvs.context
    ctx.putImageData(data, 0, 0)
    var e = new Image()
    e.src = cvs.drawable.toDataURL()

    var img;
    let url = 'data' + this.images.length + 1
    this.images[url] = e
    img = {};
    img.drawable = e
    img.drawable.src = cvs.drawable.toDataURL();
    cvs.drawable.remove()
    img.url = url;
    img.filter = "default";
    img.wrap = "clamp"
...
love.graphics.getImageData = function (x, y, w, h) {
...
love.audio.newSource = function (url, f) {
    // preload audio
    if (!this.sources[url]) {
        var e = new Audio();
        e.onload = function(){
    ...
    var source;
    source = {};
    source.audio = this.sources[url];//new Audio();
...
love.audio.loadSource = function(url) {
    this.sources_loaded[url] = this.sources_loaded[url] || this.newSource(url)
    return this.sources_loaded[url]
}
//Keyboard
...
love.mouse._move = function (event) {
    let dx = love.mouse.x - event.clientX
    let dy = love.mouse.y - event.clientY
    love.mouse.x = event.clientX;
    love.mouse.y = event.clientY;
    if (love.mousemoved) {
        love.mousemoved(love.mouse.x, love.mouse.y, dx, dy)
    }
}
...
love.loop = function (time) {
    mouseX = love.mouse.x
    mouseY = love.mouse.y
    width = love.graphics.canvas.width
    height = love.graphics.canvas.height
    cx = width/2  
    cy = height/2
...
function resize(){
    love.graphics.canvas.width = window.innerWidth
    love.graphics.canvas.height = window.innerHeight
    if (love.resize) {
        love.resize(window.innerWidth, window.innerHeight)
    }
}
...
function loadSound(url){
    return love.audio.newSource(url)
}

function drawImage(im, ix, iy, iw, ih, origin, a){
    origin = origin || {x: 0, y: 0}
    a = a || 0
    let [ow, oh] = im.getDimensions()
    let sx = iw/ow 
    let sy = ih/oh
    love.graphics.draw(im, ix, iy, a, sx, sy, ow * origin.x, oh* origin.y)
}
