class listView extends moveClip{
    ...
    drawafter(){
        ...
        if (hasBar && this.moving) {
            if (this.direction == 'v') {
                ...
                lg.line(bx, -oy + v, bx, -oy + v + sizeBar)
            }
            if (this.direction == 'h') {
                ...
                lg.line(-ox + v, by, -ox + v + sizeBar, by)
            }
        }
    }
    ...
    m_update(){
        ...
        let kindex = Math.floor((-this.offset + itemSize)/itemSize)
        //print(kindex, angle_item,this.offset)
        this.offset -= this.acc
        this.acc *= this.dump

        let rang = [0,  this.getOffset((this.items.length - this.displayItems+1))]
        if (this.limited) {
